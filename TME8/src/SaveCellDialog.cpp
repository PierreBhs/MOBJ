...
SaveCellDialog::SaveCellDialog(QWidget* parent): QDialog(parent), lineEdit_(nullptr) {
    ...
    connect(okButton, SIGNAL(clicked()), this, SLOT(accept())); //signal -> envoie slot->reception
...
bool SaveCellDialog::run(QString& name) {
    setCellName(name);
    int dialogResult = this->exec(); // exec du dialog this
    name = getCellName();
